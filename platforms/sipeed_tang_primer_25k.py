...
_dock_io = [
    ...
    # Buttons.
    ("btn_n", 0, Pins("J1:37"), IOStandard("LVCMOS33"), Misc("PULL_MODE=DOWN")),
    ("btn_n", 1, Pins("J1:39"), IOStandard("LVCMOS33"), Misc("PULL_MODE=DOWN")),
...
def pmod_led(conn="j4"):
    return [
        ("pmod_led", 0, 
            Pins(f"{conn}:0 {conn}:4 {conn}:1 {conn}:5 {conn}:2 {conn}:6 {conn}:3 {conn}:7"),
            IOStandard("LVCMOS33")
        ),
    ]
...
class Platform(GowinPlatform):
    ...
    def __init__(self, toolchain="gowin"):
        ...
        GowinPlatform.__init__(self, "GW5A-LV25MG121NC1/I0", _io, _connectors, toolchain=toolchain, devicename="GW5A-25B")
